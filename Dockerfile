...
ARG CODEX_CLI_RELEASE_VERSION="latest"

WORKDIR "${WORKSPACE_ROOT_DIR}"

# OCI Standard Labels
# https://github.com/opencontainers/image-spec/blob/main/annotations.md
LABEL org.opencontainers.image.description="OpenAI codex CLI container and tooling"
...
RUN if ! getent passwd ${CONTAINER_USER_ID} > /dev/null 2>&1; then \
  ...
  && curl -fsSL "https://raw.githubusercontent.com/openai/codex/refs/heads/main/scripts/install/install.sh" \
      | CODEX_INSTALL_DIR=/usr/local/bin CODEX_HOME=${WORKSPACE_ROOT_DIR} bash -s -- --release "${CODEX_CLI_RELEASE_VERSION}" \
